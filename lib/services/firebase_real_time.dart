...
class FirebaseRealTimeDB {
  ...
  Future<bool> addDevice(String id, String pass, String userId, String name, int selected) async {
    ...
    await _database.child('cars').child(id).update({
      'owners': val,
      'name': name,
      'image': selected
    });
  ...
  Future<List<Car>> getData() async {
    ...
    List<Car>? devicesList = devicesMap?.entries.map((entry) {
  ...
  void putSupportMessage(String topic, String message) {
    ...
    _database.child('support').child(DateTime.now().millisecondsSinceEpoch.toString()).set(node);
  }
...
//   await _database.child('devices').child(id).remove();
// }
}
